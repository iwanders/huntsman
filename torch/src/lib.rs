...
impl Canvas
{
    ...
    {
        self.pixels.iter_mut().map(|x| { *x = Default::default(); }).collect()
    }
...
impl ops::Add<Canvas> for Canvas {
    ...
    fn add(self, rhs: Canvas) -> Canvas {
        ...
        res.wipe();
        let _ = self.iter().zip(rhs.iter()).zip(res.iter_mut()).map( | ((l, r), out) | {
            *out = *l + *r;
        }).collect::<Vec<_>>();
        res
    }
...
impl State for BasicState
{
    ...
    fn get_time(&self) -> f64
    {
        ...
        return (since_the_epoch.as_secs() as f64) + (since_the_epoch.subsec_nanos() as f64 / 1_000_000_000.0);
    }
    fn get_canvas(&self) -> Canvas
    {
...
impl Torch
{
    ...
    {
        let mut mystate: BasicState = BasicState{ stored: Default::default(), base_canvas: Canvas::new(10, 10)};
...
impl Effect for AdditionEffect
{
    ...
    fn update(&mut self, state: &mut dyn State) -> Canvas
    {
        ...
        let mut child_states = self.children.iter_mut().map(|x| { x.update(state) }).collect::<Vec<Canvas>>();
