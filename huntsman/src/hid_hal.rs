...
impl HidApiHal
{
    ...
    pub fn new() -> Result<HidApiHal, String>
    {
        match hidapi::HidApi::new()
        {
            ...
            Ok(api) => Ok(HidApiHal{api: api, connected_device: None})
        }
    }

    /// Connect to a particular usb device and endpoint id.
    pub fn connect(&mut self, vendor_id: u16, product_id: u16, endpoint_id: u32) -> Result<(), String>
    {
        for device in self.api.device_list()
        {
            if (device.vendor_id() == vendor_id && device.product_id() == product_id && device.interface_number() == endpoint_id as i32)
            {
    ...
    pub fn control(&mut self, payload: &[u8]) -> Result<(), String>
    {
        match &mut self.connected_device
        {
            ...
            Some(d) =>
            {
                match d.send_feature_report(prepend_zero(payload).as_slice())
                {
                    Err(e) => Err(e.to_string()),
                    Ok(()) => Ok(()),
                }
